...
def main():
    ...
    # ===== 路径配置 =====
    base_model_path = "/home/u2022310886/jupyterlab/RL/models"
    train_path = "/home/u2022310886/jupyterlab/RL/math_lighteval_train_fitted.jsonl"
    save_dir = "/home/u2022310886/jupyterlab/RL/models/Qwen2.5-Math-1.5B-SFT-LoRA_allatention"
    ...
    # ===== 决定从哪里恢复 =====
    history_path = os.path.join(save_dir, "sft_history.json")
    has_checkpoint = os.path.exists(history_path) and os.path.exists(save_dir)

    if has_checkpoint:
        print(f"Loading existing LoRA adapter from: {save_dir}")
        model = PeftModel.from_pretrained(base_model, save_dir, is_trainable=True)
    else:
