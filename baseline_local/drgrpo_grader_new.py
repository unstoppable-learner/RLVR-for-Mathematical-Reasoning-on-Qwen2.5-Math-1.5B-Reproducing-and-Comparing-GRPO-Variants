...
def sanitize_answer_keep_latex_digits_brackets(text: str) -> str:
    if text is None:
        return ""
    ...
    mapping = {}
    for i, cmd in enumerate(_LATEX_COMMANDS_TO_KEEP):
        pattern = rf"\\{cmd}\b"
        token = f"__LATEXCMD{i}__"
        text = re.sub(pattern, token, text)
        mapping[token] = "\\" + cmd

    text = re.sub(r"[^0-9\(\)\[\]\{\}\./,\-\sA-Z_]", "", text)

    for token, cmd in mapping.items():
        text = text.replace(token, cmd)
...
def r1_zero_reward_fn(response, ground_truth, fast=True):
    ...
    if "</think> <answer>" in response and "</answer>" in response:
        ...
        cleaned_model_answer = sanitize_answer_keep_latex_digits_brackets(model_answer)

        if isinstance(ground_truth, str):
            is_correct = grade(model_answer, ground_truth, fast)
            if not is_correct:
                cleaned_gt = sanitize_answer_keep_latex_digits_brackets(ground_truth)
                is_correct = grade(cleaned_model_answer, cleaned_gt, fast)
            ...
            if not is_correct:
                for gt in ground_truth:
                    cleaned_gt = sanitize_answer_keep_latex_digits_brackets(str(gt))
                    if grade(cleaned_model_answer, cleaned_gt, fast):
                        is_correct = True
                        break
        if is_correct:
            return {
