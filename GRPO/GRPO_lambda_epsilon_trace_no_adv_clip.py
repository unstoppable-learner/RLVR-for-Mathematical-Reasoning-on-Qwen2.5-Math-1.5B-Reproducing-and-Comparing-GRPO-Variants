...
from SFT import tokenize_prompt_and_output, truncate_to_answer_end
...
def forward_kl_old_to_policy(
    ...
    这里沿用你当前 pure-KL 代码的 KL 路径；
    ε-trace 的改动只发生在 policy surrogate objective。
    """
...
def build_trace_weights(
    ...
    """
    返回 [L, L] 下三角 trace 权重矩阵。

    recent:
        w[t, k] = (gamma * lambda)^(t-k),  k <= t

    both:
        对应论文中的
        tr(t, l) = max((gamma*lambda)^l, (gamma*lambda)^(t-l))
        这里 k = t-l，因此
        w[t, k] = max((gamma*lambda)^(t-k), (gamma*lambda)^k),  k <= t
    """
    if length <= 0:
        return torch.zeros((0, 0), dtype=dtype, device=device)

    gl = torch.tensor(gamma * trace_lambda, dtype=dtype, device=device)
    pos = torch.arange(length, device=device)
    t = pos[:, None]
    k = pos[None, :]
    valid = (k <= t)

    if trace_style == "recent":
        offset = (t - k).clamp_min(0)
        weights = torch.pow(gl, offset)
    elif trace_style == "both":
        offset = (t - k).clamp_min(0)
        recent_part = torch.pow(gl, offset)
        early_part = torch.pow(gl, k)
        weights = torch.maximum(recent_part, early_part)
    else:
        raise ValueError(f"Unsupported trace_style: {trace_style}")

    weights = weights * valid.to(dtype)
    return weights


def compute_epsilon_trace_log_ratio(
    ...
    """
    计算 ε-trace 所需的累计 log-ratio。

    注意：为了不把 prompt token 混进 trace，这里按每个样本的 response token 子序列单独计算，
    再 scatter 回原始 [B, T] 位置。
    """
    log_ratio = policy_log_probs - old_log_probs
    mask_bool = response_mask.bool()
    out = torch.zeros_like(log_ratio)

    for b in range(log_ratio.size(0)):
        idx = torch.nonzero(mask_bool[b], as_tuple=False).squeeze(-1)
        if idx.numel() == 0:
            continue
        ...
        if trace_style == "recent":
            running = torch.zeros((), dtype=lr.dtype, device=lr.device)
            z = torch.empty_like(lr)
            gl = gamma * trace_lambda
            for j in range(length):
                running = lr[j] + gl * running
                z[j] = running
        elif trace_style == "both":
            weights = build_trace_weights(
                length=length,
                gamma=gamma,
...
def compute_grpo_lambda_loss(
    ...
    这里不再对 negative advantage 做 clamp。

        ratio_trace_t = exp( sum trace_weight * (log pi_theta - log pi_old) )
        pg_obj_t = min( ratio_trace_t * delta_t,
                        clip(ratio_trace_t) * delta_t )
        其中 delta_t 用 A_NAE 近似。

    这里仍然沿用你当前代码里的 KL(old_policy || policy) 正则路径；
    也就是把 pure-KL 版本中的 PG surrogate 改成论文里的 ε-trace surrogate。
    """
    mask_f = response_mask.float()

    advantages_used = advantages.expand_as(policy_log_probs)

    trace_log_ratio = compute_epsilon_trace_log_ratio(
    ...
    ratios = torch.exp(trace_log_ratio)
    clipped_ratios = torch.clamp(ratios, clip_low, clip_high)

    surr1 = ratios * advantages_used
    surr2 = clipped_ratios * advantages_used
    pg_obj = torch.minimum(surr1, surr2)
    ...
    metadata = {
        "clip_fraction": token_was_clipped.float().sum().detach() / mask_f.sum().clamp_min(1.0),
        "kl_mean": (kl_per_token * mask_f).sum().detach() / mask_f.sum().clamp_min(1.0),
        "ratio_mean": (ratios * mask_f).sum().detach() / mask_f.sum().clamp_min(1.0),
        "sampled_prob_mean": (policy_log_probs.exp() * mask_f).sum().detach() / mask_f.sum().clamp_min(1.0),
        "adv_mean": (advantages_used * mask_f).sum().detach() / mask_f.sum().clamp_min(1.0),
        "trace_log_ratio_mean": (trace_log_ratio * mask_f).sum().detach() / mask_f.sum().clamp_min(1.0),
    }
...
def grpo_microbatch_train_step(
    ...
    这里将纯 KL 版本的策略目标替换为 ε-trace 类型的 GRPO-λ。
    """
    per_token_loss, loss_meta = compute_grpo_lambda_loss(
...
def grpo_step(
    ...
    for micro_idx in range(gradient_accumulation_steps):
        ...
        if "ratio_mean" in metadata:
            total_ratio_mean += float(metadata["ratio_mean"].item())
            ratio_mean_count += 1

        if "adv_mean" in metadata:
            total_adv_mean += float(metadata["adv_mean"].item())
            adv_mean_count += 1
    ...
    if ratio_mean_count > 0:
        metrics["ratio_mean"] = total_ratio_mean / ratio_mean_count
    if adv_mean_count > 0:
        metrics["adv_mean"] = total_adv_mean / adv_mean_count
...
# =========================
# main GRPO-lambda loop
# =========================
def algorithm3_grpo_lambda(
    model,
    tokenizer,
    optimizer,
    scheduler,
    ...
    for local_step in range(1, n_grpo_steps + 1):
        ...
        if len(step_metrics_accum) > 0 and any("ratio_mean" in m for m in step_metrics_accum):
            metrics["ratio_mean"] = (
                sum(m.get("ratio_mean", 0.0) for m in step_metrics_accum)
                / max(len(step_metrics_accum), 1)
            )
        if len(step_metrics_accum) > 0 and any("adv_mean" in m for m in step_metrics_accum):
            metrics["adv_mean"] = (
                sum(m.get("adv_mean", 0.0) for m in step_metrics_accum)
                / max(len(step_metrics_accum), 1)
            )
...
def main():
    ...
    save_dir = "/home/u2022310886/jupyterlab/RL/models/Qwen2.5-Math-1.5B-SFT-GRPO-lambda-epsTrace-0.5beta"
    ...
    model, history = algorithm3_grpo_lambda(
        model=model,
        tokenizer=tokenizer,
        optimizer=optimizer,
        scheduler=scheduler,
