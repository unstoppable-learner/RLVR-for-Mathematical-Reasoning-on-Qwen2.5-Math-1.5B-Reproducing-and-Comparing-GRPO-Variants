...
def build_proxy_distribution_from_log_probs(
    log_probs_all: torch.Tensor,
    min_p_ratio: float = 0.02,
    fixed_tau: float | None = None,
):
    """
    根据 old policy 的全分布构造 proxy 分布。
    log_probs_all: (B, T, V)

    返回:
        proxy_probs: (B, T, V)
        keep_mask:   (B, T, V)  表示哪些 token 被保留
        tau:         (B, T, 1)
    """
    probs = log_probs_all.exp()

    if fixed_tau is not None:
        tau = torch.full_like(probs[..., :1], fixed_tau)
    else:
        # min-p: tau = kappa * max_prob
        tau = min_p_ratio * probs.max(dim=-1, keepdim=True).values

    keep_mask = probs > tau
    kept_probs = probs * keep_mask

    denom = kept_probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
    proxy_probs = kept_probs / denom

    return proxy_probs, keep_mask, tau


def forward_kl_proxy_to_policy(
    proxy_probs: torch.Tensor,
    ...
    返回形状: (B, T)
    """
    proxy_log_probs = torch.where(
        proxy_probs > 0,
        torch.log(proxy_probs.clamp_min(1e-12)),
        torch.zeros_like(proxy_probs),
    )

    kl = (proxy_probs * (proxy_log_probs - policy_log_probs_all)).sum(dim=-1)
    return kl


def get_low_prob_threshold(
    selected_probs: torch.Tensor,
    response_mask: torch.Tensor,
    rho: float = 0.01,
):
    """
    计算当前 batch 内 sampled token 概率的最低 rho 分位数 δ^B_ρ
    """
    valid = selected_probs[response_mask.bool()]
    if valid.numel() == 0:
        return torch.tensor(
            0.0,
    ...
    # quantile 需要 float/double
    valid_fp32 = valid.float()
    q = torch.quantile(valid_fp32, rho)

    # 返回到原 dtype，方便后面和 policy_probs 比较
    return q.to(selected_probs.dtype)
...
def compute_lpreg_loss(
    advantages: torch.Tensor,                 # (B, 1)
    policy_log_probs_all: torch.Tensor,       # (B, T, V)
    policy_log_probs: torch.Tensor,           # (B, T) sampled token
    policy_probs: torch.Tensor,               # (B, T) sampled token
    old_log_probs: torch.Tensor,              # (B, T) sampled token
    proxy_probs: torch.Tensor,                # (B, T, V)
    proxy_selected_probs: torch.Tensor,       # (B, T) sampled token prob under proxy
    ...
    双边 clip 版 Lp-Reg:
      ...
      - beta * I[low_prob & not_noise & neg_adv] * KL(proxy || policy)
    """
    ratios = torch.exp(policy_log_probs - old_log_probs)              # (B, T)
    clipped_ratios = torch.clamp(ratios, clip_low, clip_high)         # 改成双边界 [0.8, 1.2]

    pg_obj = clipped_ratios * advantages                              # (B, T)

    kl_per_token = forward_kl_proxy_to_policy(
        proxy_probs=proxy_probs,
        policy_log_probs_all=policy_log_probs_all,
    )                                                                 # (B, T)

    delta = get_low_prob_threshold(
        selected_probs=policy_probs,
        response_mask=response_mask,
        rho=low_prob_rho,
    )

    low_prob = policy_probs < delta
    not_noise = proxy_selected_probs > 0
    neg_adv = advantages < 0

    indicator = low_prob & not_noise & neg_adv & response_mask.bool()

    per_token_obj = pg_obj - beta * indicator.float() * kl_per_token
    loss = -per_token_obj
    ...
    metadata = {
        ...
        "lpreg_fraction": indicator.float().sum().detach()
                          / response_mask.float().sum().clamp_min(1.0),
        "delta_prob": delta.detach(),
        "sampled_prob_mean": (policy_probs * response_mask.float()).sum().detach()
                             / response_mask.float().sum().clamp_min(1.0),
        "proxy_selected_prob_mean": (proxy_selected_probs * response_mask.float()).sum().detach()
                                    / response_mask.float().sum().clamp_min(1.0),
    }
...
def grpo_microbatch_train_step(
    ...
    proxy_probs: torch.Tensor,
    proxy_selected_probs: torch.Tensor,

    lpreg_beta: float,
    lpreg_clip_low: float,
    lpreg_clip_high: float,
    lpreg_low_prob_rho: float,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    """
    Execute a forward-and-backward pass on one GRPO microbatch.
    这里只保留 lp_reg 路径。
    """
    per_token_loss, loss_meta = compute_lpreg_loss(
        ...
        proxy_probs=proxy_probs,
        proxy_selected_probs=proxy_selected_probs,
        response_mask=response_mask,
        beta=lpreg_beta,
        clip_low=lpreg_clip_low,
        clip_high=lpreg_clip_high,
        low_prob_rho=lpreg_low_prob_rho,
    )
...
def grpo_step(
    ...
    lpreg_beta: float = 0.5,
    lpreg_clip_low: float = 0.8,
    lpreg_clip_high: float = 1.2,
    lpreg_low_prob_rho: float = 0.01,
    ...
    total_lpreg_fraction = 0.0
    lpreg_fraction_count = 0

    total_delta_prob = 0.0
    delta_prob_count = 0
    ...
    for micro_idx in range(gradient_accumulation_steps):
        ...
        policy_probs = forward_out["selected_probs"]
        ...
        micro_proxy_probs = None
        if proxy_probs is not None:
            micro_proxy_probs = proxy_probs[start:end].to(device)
            micro_proxy_probs = micro_proxy_probs[:, :policy_log_probs.size(1), :]

        micro_proxy_selected_probs = None
        if proxy_selected_probs is not None:
            micro_proxy_selected_probs = proxy_selected_probs[start:end].to(device)
            micro_proxy_selected_probs = micro_proxy_selected_probs[:, :policy_log_probs.size(1)]
        ...
        loss, metadata = grpo_microbatch_train_step(
            ...
            proxy_probs=micro_proxy_probs,
            proxy_selected_probs=micro_proxy_selected_probs,

            lpreg_beta=lpreg_beta,
            lpreg_clip_low=lpreg_clip_low,
            lpreg_clip_high=lpreg_clip_high,
            lpreg_low_prob_rho=lpreg_low_prob_rho,
        )
        ...
        if "lpreg_fraction" in metadata:
            total_lpreg_fraction += float(metadata["lpreg_fraction"].item())
            lpreg_fraction_count += 1

        if "delta_prob" in metadata:
            total_delta_prob += float(metadata["delta_prob"].item())
            delta_prob_count += 1
    ...
    if lpreg_fraction_count > 0:
        metrics["lpreg_fraction"] = total_lpreg_fraction / lpreg_fraction_count
    if delta_prob_count > 0:
        metrics["delta_prob"] = total_delta_prob / delta_prob_count
...
def algorithm3_grpo(
    ...
    save_dir: str | None = None,      # 新增
    save_every: int = 50,             # 新增
    lpreg_beta: float = 0.5,
    lpreg_min_p_ratio: float = 0.02,
    lpreg_fixed_tau: float | None = None,
    lpreg_clip_low: float = 0.8,
    lpreg_clip_high: float = 1.2,
    lpreg_low_prob_rho: float = 0.01,
    ...
    for local_step in range(1, n_grpo_steps + 1):
        ...
        cached_proxy_probs = None
        cached_proxy_selected_probs = None

        with torch.inference_mode():
            old_log_probs_chunks = []
            proxy_probs_chunks = []
            proxy_selected_probs_chunks = []
            ...
            for start in range(0, len(rollout_prompts), old_cache_microbatch_size):
                ...
                proxy_probs, _, _ = build_proxy_distribution_from_log_probs(
                    log_probs_all=old_out["log_probs_all"],
                    min_p_ratio=lpreg_min_p_ratio,
                    fixed_tau=lpreg_fixed_tau,
                )
        
                proxy_selected_probs = torch.gather(
                    proxy_probs,
                    dim=-1,
                    index=labels.unsqueeze(-1),
                ).squeeze(-1)
        
                proxy_probs_chunks.append(proxy_probs.detach().cpu())
                proxy_selected_probs_chunks.append(proxy_selected_probs.detach().cpu())
        ...
        cached_proxy_probs = pad_and_stack_3d(proxy_probs_chunks, pad_value=0.0)
        cached_proxy_selected_probs = pad_and_stack_2d(proxy_selected_probs_chunks, pad_value=0.0)
        ...
        for epoch in range(epochs_per_rollout_batch):
            ...
            for start in range(0, rollout_batch_size, train_batch_size):
                ...
                batch_proxy_probs = None
                if cached_proxy_probs is not None:
                    batch_proxy_probs = cached_proxy_probs[batch_idx]

                batch_proxy_selected_probs = None
                if cached_proxy_selected_probs is not None:
                    batch_proxy_selected_probs = cached_proxy_selected_probs[batch_idx]
                ...
                train_metrics = grpo_step(
                    ...
                    proxy_probs=batch_proxy_probs,
                    proxy_selected_probs=batch_proxy_selected_probs,
        ...
        if len(step_metrics_accum) > 0:
            if any("lpreg_fraction" in m for m in step_metrics_accum):
                metrics["lpreg_fraction"] = (
                    sum(m.get("lpreg_fraction", 0.0) for m in step_metrics_accum)
                    / max(len(step_metrics_accum), 1)
                )
            if any("delta_prob" in m for m in step_metrics_accum):
                metrics["delta_prob"] = (
                    sum(m.get("delta_prob", 0.0) for m in step_metrics_accum)
                    / max(len(step_metrics_accum), 1)
                )
        sampled_level_count = {}
        ...
        print(
            ...
            f"lpfrac={metrics.get('lpreg_fraction', 0.0):.4f}, "
...
def main():
    ...
    save_dir = "/home/u2022310886/jupyterlab/RL/models/Qwen2.5-Math-1.5B-SFT-FullGRPO-oldclip-KL"
    ...
    lpreg_beta = 0.5
    lpreg_min_p_ratio = 0.02
    lpreg_fixed_tau = None
    lpreg_clip_low = 0.8
    lpreg_clip_high = 1.2
    lpreg_low_prob_rho = 0.05
    ...
    model, history = algorithm3_grpo(
        ...
        save_dir=save_dir,      # 新增
        save_every=50,          # 新增
        lpreg_beta=lpreg_beta,
        lpreg_min_p_ratio=lpreg_min_p_ratio,
        lpreg_fixed_tau=lpreg_fixed_tau,
        lpreg_clip_low=lpreg_clip_low,
        lpreg_clip_high=lpreg_clip_high,
        lpreg_low_prob_rho=lpreg_low_prob_rho,
